...
这是一个可本地运行、可自部署的开源小游戏。游戏本体不依赖外部服务;只有初音未来 NPC 聊天需要配置 DeepSeek API key。
...
VOCALOID、初音未来(Hatsune Miku)、相关歌曲名、P 主名、榜单数据和任何第三方素材的权利归各自权利人所有。本仓库用于粉丝创作和技术学习;商用或再分发前请自行确认素材、数据和商标授权。
